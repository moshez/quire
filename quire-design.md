...
#### 2.3.1 Remove swiftlink-specific code

Delete entirely:

- `vapidPublicKey`, `swEventListeners`, `handleSwMessage()`
- `openPushDB()`, `PUSH_DB_NAME`, `PUSH_STORE_NAME`, `PUSH_DB_VERSION`
- `urlBase64ToUint8Array()`
- All push-related imports: `js_push_subscribe`, `js_push_get_subscription`, `js_notification_show`, `js_set_vapid_key`, `js_sw_add_listener`, `js_sw_remove_listener`, `js_get_pending_pushes`, `js_clear_pending_pushes`
- `js_window_focus`
- `EVENT_PUSH`, `EVENT_NOTIFICATION_CLICK`

#### 2.3.2 Replace OP_SET_STYLE → OP_SET_TRANSFORM
...
#### 2.3.3 Replace OP_ADD_CHILD → OP_CREATE_ELEMENT
...
#### 2.3.7 Add js_measure_node import
...
#### 2.3.9 Add decompression import
...
#### 2.3.10 Add IndexedDB key-value imports
...
#### 2.3.11 Add js_set_inner_html_from_blob import
...
#### 2.3.12 Auto-flush proxy wrapper
...
├── Makefile                  # ATS2 → C → WASM build
├── package.json              # npm scripts (dev server, bridge tests)
├── index.html                # Minimal: loading div + import
├── manifest.json             # PWA manifest
├── service-worker.js         # Offline caching
├── bridge.js                 # Generic WASM↔DOM bridge
├── reader.css                # Reader styles + CSS variables
├── src/
│   ├── runtime.c             # Minimal WASM C runtime (malloc, buffers)
│   ├── quire.sats            # Top-level static declarations
│   ├── quire.dats            # Top-level implementation (init, event dispatch)
│   ├── dom.sats              # DOM type model (dataprops)
│   ├── dom.dats              # Diff emission
│   ├── epub.sats             # EPUB/ZIP parsing types
│   ├── epub.dats             # EPUB processing
│   ├── reader.sats           # Reader state machine types
│   └── reader.dats           # Sliding window, pagination
├── build/                    # Generated files (gitignored)
│   ├── *_dats.c              # ATS2 → C output
│   └── quire.wasm            # Final WASM binary
└── test/
...
- [ ] **M7: Bridge file + blob + decompression imports**
  - Add `js_file_open`, `js_file_read_chunk`, `js_file_close` (§2.3.8)
  - Add `js_decompress`, `js_blob_read_chunk`, `js_blob_size`, `js_blob_free` (§2.3.9)
  - Add `js_set_inner_html_from_blob` (§2.3.11)
  - Bridge-only PR — no app logic

- [ ] **M8: Bridge IndexedDB imports**
  - Add `js_kv_open`, `js_kv_put`, `js_kv_put_blob`, `js_kv_get`, `js_kv_delete` (§2.3.10)
  - Bridge-only PR
