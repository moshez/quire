...
(* witness_toc_visible: produces TOC_STATE(true).
 * Requires n:pos — caller MUST have proved toc_mode > 0 via gt1_int_int.
 * The {n:pos} constraint is type-system-enforced, not merely conventional. *)
fn witness_toc_visible{n:pos}(v: int(n)): (TOC_STATE(true) | void) =
  (unit_p() | ())
...
in
  if gt1_int_int(mode, 0) then let
    val (pf_open | ()) = witness_toc_visible(mode)
 ...
 * Uses gt1_int_int as outer dispatch to establish {n:pos} for witness. *)
...
(* ESCAPE_ACTION proves which layer was popped WITH proof that the action occurred:
 * - ESCAPE_CLOSED_TOC: carries TOC_STATE(false) — only obtainable from hide_toc_panel
 * - ESCAPE_HID_CHROME: no available proof token for chrome hiding
 * - ESCAPE_EXITED_READER: carries POSITION_SAVED() — only obtainable from library_update_position *)
dataprop ESCAPE_ACTION =
  | ESCAPE_CLOSED_TOC of TOC_STATE(false)
  | ESCAPE_HID_CHROME
  | ESCAPE_EXITED_READER of POSITION_SAVED()
...
fn handle_escape{n:nat}(toc_mode: int(n), chrome_vis: int, root_id: int)
  : (ESCAPE_ACTION | void) =
  if gt1_int_int(toc_mode, 0) then let
    val (pf_open | ()) = witness_toc_visible(toc_mode)
    val (pf_closed | ()) = hide_toc_panel(pf_open | )
  in (ESCAPE_CLOSED_TOC(pf_closed) | ()) end
  else if eq_int_int(chrome_vis, 1) then let
    val () = hide_chrome()
    val _ = reader_incr_chrome_timer_gen()
  in (ESCAPE_HID_CHROME() | ()) end
  else let
  ...
  in (ESCAPE_EXITED_READER(pf_saved) | ()) end
  ...
  in
    if eq_int_int(key_len, 6) then
      ...
      if eq_int_int(k0, 69) then let
        val toc_mode = reader_get_toc_view_mode()
        val chrome_vis = reader_get_chrome_visible()
        val (pf_esc | ()) = handle_escape(toc_mode, chrome_vis, root_id)
        prval _ = pf_esc
      in end
      else ()
